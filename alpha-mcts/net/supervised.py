...
batch_size = 256
train_set = FastChessDataset("alpha-mcts/net/training_data/lichess_elite_2022-02.jsonl", 4000, 8000)
...
for epoch in range(5):
    ...
    for i, data in enumerate(train_loader, 1):
        ...
        if i%400 == 0:
            print(f"Epoch: {epoch + 1}, Batch number: {i}, Policy loss: {cur_policy_loss/400:.4f}, Value Loss: {cur_value_loss/400:.4f}")
            print(f"GPU temperature: {torch.cuda.temperature()} degrees")
            x_labels.append(epoch * len(train_set) + i)
            plot_losses.append(cur_loss/400)
